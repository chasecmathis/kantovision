...
def create_team(body: SaveTeamRequest, user: UserIdDep) -> TeamRow:
    ...
    db = get_db()
    existing = db.table("teams").select("id").eq("user_id", user).execute()
...
def update_team(team_id: str, body: SaveTeamRequest, user: UserIdDep) -> TeamRow:
    db = get_db()
    existing = db.table("teams").select("id").eq("id", team_id).eq("user_id", user).execute()
    ...
    result = db.table("teams").update(payload).eq("id", team_id).eq("user_id", user).execute()
    return result.data[0]
...
def delete_team(team_id: str, user: UserIdDep) -> None:
    ...
    if not existing.data:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Team not found")
    db.table("teams").delete().eq("id", team_id).eq("user_id", user).execute()
